...
function getPages( options, maxPages ){
  ...
  var lastPage = maxPages ? Math.min( options.totalPages, options.currentPage + surround + padRight )
    : options.totalPages;
...
module.exports = function( list,
  ...
  return {
    handle : function(  req,
      ...
      query.originalSort = query.sort;
      query.originalSelect = query.select;
      ...
      query.exec = function( next ){
        query.count( function( err, count ){
          ...
          if ( options.select ){
            query.originalSelect( options.select );
          }

          if ( options.sort ){
            query.originalSort( options.sort );
          }

          query.originalExec( function( err, results ){
            if ( err ){
              return next( err );
            }
            ...
            var rtn = {
              ...
              pages       : [],
              previous    : ( 1 < currentPage ) ? ( currentPage - 1 ) : false,
